...
const page = usePage();
const notificationCount = ref(page.props.countNotifications);

// const getCountNotifications = () => {
//     axios.get(route('comunicados.count'))
//         .then(response => {
...
<template>
    <Link :href="route('comunicados.show')">
        <div
            class="relative flex h-8.5 w-8.5 items-center justify-center rounded-full border-[0.5px] border-stroke bg-gray hover:text-primary dark:border-strokedark dark:bg-meta-4 dark:text-white"
        >
            <span class="absolute -top-0.5 right-0 z-1 h-2 w-2 rounded-full bg-meta-1" v-if="notificationCount > 0">
